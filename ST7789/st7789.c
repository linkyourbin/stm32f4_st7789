...
void ST7789_Fill_Color(uint16_t color) {
...
#ifdef USE_DMA
  // 修复：正确填充16位颜色值到缓冲区，确保字节序正确
  uint16_t swapped_color = (color >> 8) | (color << 8); // 交换字节序
  ...
  // 处理剩余的行（如果有的话）
  uint16_t remaining_lines = ST7789_HEIGHT % HOR_LEN;
  if (remaining_lines > 0) {
    for (i = 0; i < ST7789_WIDTH * remaining_lines; i++) {
      disp_buf[i] = swapped_color;
    }
    ST7789_WriteData((uint8_t *)disp_buf, ST7789_WIDTH * remaining_lines * 2);
  }
#else
  uint16_t j;
